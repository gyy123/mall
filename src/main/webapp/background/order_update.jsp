  ...
  <div id="middle">
    ...
    <div id="center-column">
      ...
      <div class="select-bar">
      	<html:form action="order.do?method=updateQueryOrder">请输入订单号：
      ...
      <div class="table"><img src="img/bg-th-left.gif" width="8" height="7" alt="" class="left" /> <img src="img/bg-th-right.gif" width="7" height="7" alt="" class="right" />
        ...
        <table width="516" cellpadding="0" cellspacing="0" class="listing">
          ...
          <logic:empty name="list">
          		<tr><td colspan="4">没有查询结果</td></tr>
          </logic:empty>          
          <logic:notEmpty name="list">
          <logic:iterate id="o" name="list" indexId="index">      	
          	<tr<%=(index.intValue()%2==0)?" class='bg'":"" %>>
	            <td><bean:write name="o" property="order_SEQ"/></td>
	            <logic:equal name="o" property="status" value="1">
	            	<td>新订单</td>
	            	<html:form action="order.do?method=updateOrderStatus">
	            	<html:hidden property="order_SEQ" value="${o.order_SEQ}"/>
		            <td>
		            	<html:select property="status">
		            		<html:option value="2">已支付订单</html:option>
		            ...
		            <td>
		            	<html:textarea property="issue_cause" rows="1" cols="27"></html:textarea>
		            </td>
		            <td>
		            	<html:image src="background/img/edit-icon.gif"  onclick="return confirm('你确定要修改此订单的状态吗？')"></html:image>
		            </td>		            
		            </html:form>
	            </logic:equal>
	            <logic:equal name="o" property="status" value="2">
	            	<td>已支付订单</td>
	            	<html:form action="order.do?method=updateOrderStatus">
	            	<html:hidden property="order_SEQ" value="${o.order_SEQ}"/>
		            <td>
	      ...
	      <tr><td colspan="5">
        		...
        		<logic:notEmpty name="page_pre">
        			<%	PageBean pb=(PageBean)request.getAttribute("page_pre");
        				if(pb.getPageNow()!=1){
        			%>
        			<a href="order.do?method=queryOrders&status=1&pageNow=<%=pb.getPageNow()-1 %>"><img src="background/img/pre-page.jpg" border="0" align="absmiddle"/></a>&nbsp;
        			<%
        				}else{
        			%>
        			...
        			%>
        			<a href="order.do?method=queryOrders&status=1&pageNow=<%=pb.getPageNow()+1 %>"><img src="background/img/next-page.jpg" border="0" align="absmiddle"/></a>
        			<%	}else{
        			%>
	      		...
	      		<logic:notEmpty name="page">
	      			<%	int status = Integer.parseInt(request.getAttribute("status")+"");
        				...
        				if(pb.getPageNow()!=1){
        			%>
        			<a href="order.do?method=updateQueryOrder&status=<%=status %>&pageNow=<%=pb.getPageNow()-1 %>"><img src="background/img/pre-page.jpg" border="0" align="absmiddle"/></a>&nbsp;
        			<%
        				}else{
        			%>
        			<img src="background/img/pre-page.jpg" border="0" align="absmiddle"/>
        			...
        			<%
        				if(pb.getPageNow()!=pb.getPageCount()){
        			%>
        			<a href="order.do?method=updateQueryOrder&status=<%=status %>&pageNow=<%=pb.getPageNow()+1 %>"><img src="background/img/next-page.jpg" border="0" align="absmiddle"/></a>
        			<%	}else{
        			%>
        			<img src="background/img/next-page.jpg" border="0" align="absmiddle"/>
        			<%
